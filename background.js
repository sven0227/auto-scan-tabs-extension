...
/** Delay after switching to a profile tab before checking/clicking View profile (lets page load). */
const TAB_SWITCH_DELAY_MS = 2500;
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === "GET_LAST_GITHUB_URL") {
    chrome.storage.local.get(
      [STORAGE_KEY_LAST_GITHUB, STORAGE_KEY_LAST_GITHUB_URLS],
      (data) => {
        const urls = data[STORAGE_KEY_LAST_GITHUB_URLS] || [];
        const url =
          data[STORAGE_KEY_LAST_GITHUB] ||
          (urls.length ? urls[urls.length - 1] : null);
...
function runClickGitHubViewProfileInPage() {
  ...
  function findLink() {
    ...
    for (const span of spans) {
      ...
      const section =
        span.closest(
          '[class*="grid-container"], [class*="py-4x"], [class*="px-0"]'
        ) || span.closest("div");
      if (!section) continue;
      const viewProfile = section.querySelector(".view-profile a");
      const link =
        viewProfile || section.querySelector('a.up-n-link, a[class*="link"]');
      if (link && viewProfileText.test((link.textContent || "").trim()))
        return link;
    }
  ...
  ["pointerdown", "mousedown", "pointerup", "mouseup", "click"].forEach(
    function (type) {
      ...
      link.dispatchEvent(
        new Ctor(
          type,
          Object.assign({}, opts, { detail: type === "click" ? 1 : 0 })
        )
      );
    }
  );
...
function startClickingViewProfileOnAllProfileTabs(sendResponse) {
  ...
  chrome.tabs.query({ url: urlPattern }, (tabs) => {
    const profileTabs = tabs.filter(
      (t) => t.id && t.url && t.url.includes("upwork.com/freelancers/")
    );
    if (profileTabs.length === 0) {
      sendResponse({
        ...
        clicked: 0,
        closed: 0,
        error: "No freelancer profile tabs found",
      });
      return;
    }
    stopWatching();
    ...
    function doNext() {
      ...
      chrome.tabs.update(tab.id, { active: true }, () => {
        function checkAndClick(isRetry) {
          chrome.tabs.sendMessage(
            tab.id,
            { type: "HAS_GITHUB_VIEW_PROFILE_BUTTON" },
            (res) => {
              ...
              if (res.hasButton === false) {
                if (!isRetry) {
                  ...
                  setTimeout(() => checkAndClick(true), TAB_SWITCH_DELAY_MS);
                  return;
                }
              ...
              // Delay even when button is found so the page can settle before clicking
              setTimeout(() => {
                chrome.tabs.sendMessage(
                  tab.id,
